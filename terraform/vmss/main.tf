...
resource "azurerm_linux_virtual_machine_scale_set" "admin_vmss" {
  ...
  zones = ["1", "2", "3"]  # Azure will automatically handle fault domains and update domains within each zone

  # platform_fault_domain_count is generally not required in flexible orchestration mode.
  # If used in single-zone VMSS, it can be 1–5 to control fault domain distribution within that zone.
